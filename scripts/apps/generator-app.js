    ...
    try {
      ...
      if (isPersonName && this.currentMode === 'recipe') {
        ...
        if (selectedGenders.length > 1) {
          /**
           * Multi-Gender Generation with Retry Logic
           *
           * When multiple genders are selected, this retry-based approach ensures
           * the requested number of unique names is generated by:
           * - Generating one name at a time to avoid waste
           * - Randomly selecting from available genders for each attempt
           * - Tracking all generated names to prevent duplicates
           * - Continuing until target count is reached or MAX_ATTEMPTS is exhausted
           *
           * MAX_ATTEMPTS is set to count * 5 to allow for:
           * - Random gender distribution not perfectly matching desired ratios
           * - Some generations failing or returning duplicates
           * - Species with limited name data for certain genders
           *
           * This approach produces better results than requesting all names at once
           * because it can adapt to failures and continue with alternative genders.
           */
          const suggestions = [];
          const errors = [];
          const generatedNames = new Set(); // Track unique names across all genders

          /**
           * Maximum attempts before giving up (5x the requested count)
           * This multiplier allows for failed generations and duplicates while
           * preventing infinite loops when species have very limited name data
           * @constant {number}
           */
          const MAX_ATTEMPTS = count * 5;
          let attempts = 0;

          logDebug(`Multi-gender generation: trying to generate ${count} names from ${selectedGenders.length} genders`);

          while (suggestions.length < count && attempts < MAX_ATTEMPTS) {
            attempts++;

            // Pick a random gender for this attempt
            const randomGender = selectedGenders[Math.floor(Math.random() * selectedGenders.length)];

            logDebug(`Attempt ${attempts}/${MAX_ATTEMPTS}: trying gender "${randomGender}" (${suggestions.length}/${count} names generated so far)`);

            try {
              // Generate only 1 name at a time to avoid waste
              ...
              if (genderResult.suggestions && genderResult.suggestions.length > 0) {
                const newName = genderResult.suggestions[0].text;

                // Only add if it's not a duplicate
                if (!generatedNames.has(newName)) {
                  generatedNames.add(newName);

                  // Add gender info for color coding
                  // Only apply gender if we have gender-relevant components (firstname, title, nickname)
                  const suggestionWithGender = {
                    ...genderResult.suggestions[0],
                    gender: hasGenderRelevantComponents ? randomGender : null
                  };
                  suggestions.push(suggestionWithGender);

                  logDebug(`✓ Added unique name: "${newName}" (gender: ${randomGender})`);
                } else {
              ...
              logError(`Failed to generate name for gender ${randomGender}:`, error);
              errors.push({ code: 'generation_failed', message: error.message, gender: randomGender });
            }
          }

          // Shuffle to mix genders properly
          const shuffled = suggestions.sort(() => Math.random() - 0.5);

          // Log warning if we couldn't generate enough names
          if (shuffled.length < count) {
            logWarn(`Only generated ${shuffled.length}/${count} unique names after ${attempts} attempts. ` +
              `Some genders may have limited name data available.`);
          }
